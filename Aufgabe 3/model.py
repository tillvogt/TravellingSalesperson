...
def cheese_delivery(
    ...
    D = pl.LpVariable.dicts("d", (restaurants, [t for t in range(n_trucks)]), lowBound=0, cat=pl.LpInteger) #Variable wich splits the demands on different trucks


    #defining the objective function

    problem += pl.lpSum(pl.lpSum(X[i][j][k] * d[i][j] for i in locations for j in locations) for k in range(n_trucks)) * var_km + pl.lpSum(pl.lpSum(X[i][j][k] * t[i][j] for i in locations for j in locations) for k in range(n_trucks)) * var_min + pl.lpSum(Y[k] * fix for k in range(n_trucks)) + pl.lpSum(Z[j]*minute_penalty for j in locations)
    ...
    for k in range(n_trucks):
        ...
        problem += pl.lpSum(D[j][k] for j in restaurants) <= cap     #constraint for not exceeding capacity
        ...
        for j in restaurants:
            problem += pl.lpSum(X[i][j][k] for i in locations)*cap >= D[j][k] #this ensures that there can only be an capacity assignement, if restaurant j is included in truck ks tour
    ...
    for j in restaurants:
        ...
        problem += pl.lpSum(D[j][k] for k in range(n_trucks)) >= dem[j]  #the delivered goods of all trucks must reach the total demands of each restaurant.
    ...
    problem.solve(pl.PULP_CBC_CMD(timeLimit=1500))
